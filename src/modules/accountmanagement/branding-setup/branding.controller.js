...
// Middleware for uploading branding files
...
const createOrUpdateBranding = async (req, res) => {
  try {
    const { companyId, welcomeTitle, welcomeMessage } = req.body;
    ...
    branding = await BrandingModel.findByIdAndUpdate(branding._id, payload, {
      new: true,
    });
...
// Get branding details by companyId
